...
import { MetricsRenderManager } from '@/lib/signal-visualizer/metricsComponent/core/metricsRenderManager.ts'
...
export class MetricsContainer {
    ...
    async init(htmlElement: HTMLElement, rollingWindowMs: number = DEFAULT_ROLLING_WINDOW_MS) {
        const renderManager = new MetricsRenderManager(htmlElement, this.eventMediator)
