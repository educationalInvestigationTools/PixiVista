...
export class SignalSourceManager {
    private serializers: Map<string, SignalSourceBuildDataSerializer<SignalSourceBuildData>> = new Map()
    private signalsBuildData: Map<string, SignalSourceBuildData[]> = new Map()
    private factories: Map<string, SignalSourceFactory<SignalSourceBuildData, SignalSource>> = new Map()
    ...
    addFactory(serializerId: string, factory: SignalSourceFactory<SignalSourceBuildData, SignalSource>) {
    ...
    serialize(): SerializedOutput {
        ...
        for (const [serializerId, serializer] of this.serializers) {
            const signalsBuildData = this.signalsBuildData.get(serializerId)
            if (signalsBuildData !== undefined) {
                result.set(serializerId, signalsBuildData.map(x => serializer.serialize(x)))
            }
        }
    ...
    deSerialize(value: SerializedOutput) {
        ...
        for (const [serializerId, serializer] of this.serializers) {
            ...
            if (serializedSignals !== undefined) {
                serializedSignals.map(x => this.addSignalBuildData(serializerId, serializer.deserialize(x)))
            }
        }
    }
}
