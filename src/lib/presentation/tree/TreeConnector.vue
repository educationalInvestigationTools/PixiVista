...
<template>
    <span class="tree-connector" :style="{ width: cssWidth }">
        <svg class="tree-connector__svg" :width="svgWidth" :height="viewBoxHeight"
            :viewBox="`0 0 ${svgWidth} ${viewBoxHeight}`" preserveAspectRatio="none">
            <line v-for="(x, index) in ancestorLineXs" :key="`ancestor-${index}`" class="tree-connector__line" :x1="x"
                y1="0" :x2="x" :y2="viewBoxHeight" />
            <line class="tree-connector__line" :x1="branchX" y1="0" :x2="branchX"
                :y2="props.isLast ? midY : viewBoxHeight" />
            <line class="tree-connector__line" :x1="branchX" :y1="midY" :x2="svgWidth" :y2="midY" />
        </svg>
    </span>
</template>
